"""Tagging orchestrator — H1 rules + H2 local LLM with cache and fallback."""
...
class TaggingEngine:
    """Orchestrates H1 (fast heuristics) and H2 (LLM semantic) tagging."""
    ...
    async def tag_document(
        ...
        # H2: Run if enabled and model available
        semantic_result = {"semantic": [], "model": "", "inferred_at": "", "llm_status": "disabled"}
        if self.h2 and self.cfg.model_path:
            try:
                ...
                semantic_result = {
                    ...
                    "model": self.cfg.model_path.split("/")[-1],
                    "inferred_at": __import__("datetime").datetime.utcnow().isoformat() + "Z",
